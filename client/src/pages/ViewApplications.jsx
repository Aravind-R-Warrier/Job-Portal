...
const changeJobApplicationStatus=async(id,status)=>{
  ...
  return applicants?applicants.length===0?(
    <div className='flex items-center justify-center h-[70-vh]'>
      No Resume Available or Posted
    </div>
  ): (
            ...
            {applicants.filter(item=>item.jobId&&item.userId).map((application, index) => (
              <tr key={index} className='text-gray-700'>
                ...
                <td className='py-2 px-4 border-b  flex items-center'>
                  ...
                  <span className='text-center flex items-center'>{application.userId.name}</span>
                </td>
                <td  className='py-2 px-4 w-10 h-10 rounded-full mr-3 max-sm:hidden'>{application.jobId.title}</td>
                <td  className='py-2 px-4 w-10 h-10 rounded-full mr-3 max-sm:hidden'>{application.jobId.location}</td>
